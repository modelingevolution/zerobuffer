...
BUFFER_NAME="csharp_python_test_$$"
FRAME_COUNT=1000
FRAME_SIZE=1024
...
# Create Python test reader script (reuse from cpp-python)
    ...
    try:
        while frames_read < expected_frames:
            ...
            if frame:
                ...
                if frame.sequence != frames_read:
                    print(f"Sequence error: expected {frames_read}, got {frame.sequence}")
...
if grep -q "Read metadata:" "$SCRIPT_DIR/reader_output.txt" 2>/dev/null; then
    echo "✓ Metadata: Successfully exchanged"
fi
...
if [ "$WRITER_SUCCESS" = true ] && [ "$READER_SUCCESS" = true ]; then
    ...
    # Get throughput from both sides
    WRITER_THROUGHPUT=$(grep -oP "Throughput: \K[0-9.]+" "$SCRIPT_DIR/writer_output.txt" 2>/dev/null || echo "N/A")
    READER_THROUGHPUT=$(grep -oP "Throughput: \K[0-9.]+" "$SCRIPT_DIR/reader_output.txt" 2>/dev/null || echo "N/A")
    
    if [ "$WRITER_THROUGHPUT" != "N/A" ]; then
        echo "  Writer throughput: $WRITER_THROUGHPUT MB/s"
    fi
    if [ "$READER_THROUGHPUT" != "N/A" ]; then
