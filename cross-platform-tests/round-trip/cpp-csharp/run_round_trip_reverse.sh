...
# Parse reader results (C++)
READER_SUCCESS=false
READER_FRAMES=$(grep -oP "read \K\d+" "$SCRIPT_DIR/reader_output.txt" | tail -1 || echo "0")
READER_ERRORS=$(grep -oP "errors: \K\d+" "$SCRIPT_DIR/reader_output.txt" | tail -1 || echo "0")
...
# Check for wrap-around in C++ reader output
echo ""
echo "Wrap-around Analysis:"
EXPECTED_WRAPS=$((FRAME_COUNT / 82))
WRAP_COUNT=$(grep -c -i "wrap" "$SCRIPT_DIR/reader_output.txt" 2>/dev/null || echo "0")
